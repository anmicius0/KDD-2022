...
def traverse_wind_farm(method, params, model_path, partition):
    # type: (Callable, dict, str, int) -> None
    """
    Desc:
    ...
    Args:
        ...
        partition: the partition of the data
    Returns:
        Predictions (for test) or None
    """
    start_time = time.time()
    losses = np.array([])
    for i in range(params["per_split"] * (partition - 1), params["per_split"] * partition):
        params["turbine_id"] = i
        exp = Experiment(params)

        print('>>>>>>> Training Turbine {:3d} >>>>>>>>>>>>>>>>>>>>>>>>>>'.format(i))
        val_loss = method(exp, model_path, is_debug=params["is_debug"])
        paddle.device.cuda.empty_cache()

        losses = np.append(losses, [val_loss])
        print("Val_loss for {}: {}".format(i, val_loss))
        print('Average val_loss now till {}: {}'.format(i, np.average(losses)))

        end_time = time.time()
        print("Elapsed time for turbine {}: {}\n".format(i, end_time - start_time))
        start_time = end_time

    #
    # Grid test area
    # dropout = [0, 0.05, 0.1, 0.3]
    # batch_size = [16, 32, 64]
    # lowest_loss = 100
    # best_params = None
    # for d in dropout:
    ...
    #         exp = Experiment(params)
    #
    #         print(">>>>> Training: dropout: {}, batch_size: {}".format(d, b))
    #         current_loss = method(exp, model_path, is_debug=params["is_debug"])
    ...
    # print("lstm_layer: {}".format(best_params["lstm_layer"]))
    # print("dropout: {}".format(best_params["dropout"]))
    # print("batch_size: {}".format(best_params["batch_size"]))
    # print("lr: {}".format(best_params["lr"]))
    # print("Loss: {}".format(lowest_loss))
